--[[
    ...
    Memory fix: old spine_widget freed before replacement.
    Performance fix: widget pre-rendered on creation, opacity set once.
]]--
...
local function patchBookSpineEffect(plugin)
    ...
    function MosaicMenuItem:paintTo(bb, x, y)
        ...
        if cached_height ~= fh or cached_width ~= cover_w or cached_offset ~= offset then
            ...
            -- Free old widget before creating new one to avoid memory leak
            ...
            spine_widget = ImageWidget:new{
                ...
                width   = clipped_width,
                height  = fh,
                opacity = spine_intensity, -- set once at creation, not per draw
            }
            -- Pre-render now so paintTo is just a fast blit with no processing
            spine_widget:_render()
        ...
        if spine_widget then
            spine_widget:paintTo(bb, fx + offset, fy)
        end
    end
