--[[
    ...
    It mimics the style used by many print novels:
        The page numbers alternate between the top-left and top-right
...
ReaderView.paintTo = function(self, bb, x, y)
    ...
    --local header_font_face = "Source_Serif_4/SourceSerif4-Regular.ttf" -- this is the serif font from Project: Title
    --local header_font_face = "LMRoman/lmromancaps10-regular.otf" -- this is the serif font from Project: Title
    ...
    --local header_font_size = header_settings.text_font_size or 12 -- Will use your footer setting if available
    local header_font_bold = header_settings.text_font_bold or false -- Will use your footer setting if available
    local header_font_bold = true -- Will use your footer setting if available
    --local header_font_color = Blitbuffer.COLOR_BLACK -- black is the default, but there's 15 other shades to try
    local header_font_color = Blitbuffer.COLOR_GRAY_4 -- A nice dark gray, a bit lighter than black
    local header_top_padding = 65 --65 --Size.padding.small -- replace small with default or large for more space at the top
    ...
    local header_margin = 40 -- Use this instead, if book margins is set to false
    local left_max_width_pct = 14 -- this % is how much space the left corner can use before "truncating..."
    local right_max_width_pct = 14 -- this % is how much space the right corner can use before "truncating..."
    local header_max_width_pct = 72 -- this % is how much space the header can use before "truncating..."
    local separator = {
        bar     = "|",
        bullet  = "•",
        dot     = "·",
    ...
    if (pages_done > 1) and (pageno % 2 == 0) then
        centered_header = string.format("%s %s %s", book_author, separator.bullet, book_title)
    elseif (pages_done > 1) and (pageno % 2 ~= 0) then
